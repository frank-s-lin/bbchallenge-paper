...
\CoqBBnospace's \ngramcps core algorithm, without augmentations, was first developed in the Rust programming language by bbchallenge's contributor Nathan Fenner \cite{ngramcps_fenner}. \tsm{It's unclear if we're gonna ack bbchallenge contributors like that or in an entire section dedicated to that purpose.}
