\subsection{Pipelines and overview}\label{sec:pipelines}

\subsubsection{Pipeliens}
...
There are essentially five deciders that were developed to solve $S(5)$, see Table~\ref{tab:pipelineBB5}: loops, $n$-gram Closed Position Word List (NGramCPS), Repeated Word List (RepWL), Finite Automata Reduction (FAR), Weighted Finite Automata Reduction (WFAR) and they are described in (Sections~\ref{sec:loops} to \ref{sec:WFAR}). All these deciders are orignal\footnote{There existed a previous algorithm to decide loops \cite{Lin1963}, but we present a new one.} and were developed by the bbchallenge collaboration. Loops, RepWL and augmentations of NGramCPS (see Section~\ref{sec:n-gramCPS}) are original to \CoqBB while the other deciders have been implemented in various programming languages before they were integrated in \CoqBB.

Solving $S(2,4)$ and previously known $S(4)$ only used a subset of these deciders (see Tables~\ref{tab:pipelineBB2x4}~and~\ref{tab:pipelineBB4}) and required a lot less compute and overall effort -- \eg no individual nonhalting proofs, in contrast with 5-state sporadic machines, Section~\ref{sec:sporadic}.

All these deciders can be expressed in the same general framework, known as Closed Tape Language (CTL) which is attributed to Marxen and Buntrock but was first documented by Ligocki \cite{ShawnCTL}:
...
\paragraph{Deciders and verifiers.}
