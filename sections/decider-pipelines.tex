...
Solving $S(2,4)$ and previously known $S(4)$ \cite{Brady83} only used a subset of these deciders (see Tables~\ref{tab:pipelineBB2x4}~and~\ref{tab:pipelineBB4}) and required a lot less compute and overall effort -- \eg no individual nonhalting proofs, in contrast with 5-state sporadic machines, Section~\ref{sec:sporadic}.
