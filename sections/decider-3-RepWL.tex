...
\paragraph{Regex branching.} When the head is facing a block with a $^+$, for instance in Figure~\ref{fig:repWL} we have $\texttt{0}^\infty \; \texttt{01}^1 \; \texttt{D>} \; (\texttt{01})^{3+} \; \texttt{0}^\infty$, from which we add two configurations to the set of configurations to visit next:
\begin{enumerate}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \State{\textbf{Input:} A Turing machine $\mathcal{M}$, block length parameter $l>0$, repeat threshold $T>0$, maximum number of steps allowed in block simulation $B \in \mathbb{N}$, maximum number of distinct nodes we are willing to visit $N\in \mathbb{N}$.}
        ...
        \State \While{$|V| < N$ \textbf{and} $|\texttt{to\_visit}| \neq 0$}
