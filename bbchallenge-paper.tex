...
\begin{itemize}
    ...
    \item $S(3,3)$: \tm{1RB1RA_0LC1LE_1LD1LC_1LA0LB_1LF1RE_---0RA}; \textbf{Bigfoot}.\footnote{\label{note:bigfoot}\url{https://wiki.bbchallenge.org/wiki/Bigfoot}} Take the following parametrised configuration:
          ...
          The machine enters configuration $A(2,1,2)$ at step 69 and halts if and only if $a$ reached $0$ in the case $b = 2 \text{ mod } 6$ \cite{sligocki2023bb33}. Bigfoot has also been compiled into a 7-state 2-symbol machine: \tm{0RB1RB_1LC0RA_1RE1LF_1LF1RE_0RD1RD_1LG0LG_---1LB}. A probabilistic model\footnotemark[\getrefnumber{note:bigfoot}] suggests that the chances of the machine ever halting are less than $2^{-3999889}$.
    ...
    \item $S(2,5)$: \tm{1RB3RB---3LA1RA_2LA3RA4LB0LB0LA}; \textbf{Hydra}.\footnote{\label{note:hydra}\url{https://wiki.bbchallenge.org/wiki/Hydra}} Take the following parametrised configuration:
          ...
          The machine reaches $A(3,0)$ after 20 steps and halts if and only if $(a,b)$ with $a$ even and $b=0$ is ever reached. This can be reformulated as whether repeatedly applying the function $f(n) = 3\lfloor\frac{n}{2}\rfloor +3$ from $n=4$ will at some point produce more even values of $n$ than twice the number of odd values. A probabilistic model\footnotemark[\getrefnumber{note:hydra}] suggests that the chances of the machine ever halting are less than $\left( \frac{\sqrt{5} - 1}{2} \right)^{2005374}$.
