...
% \Coq is also a fully-fledged programming language and the way \CoqBB works is to implement both the TNF enumeration and the deciders (see proof structure above) directly in Coq, prove them correct, run them and use the outputs for concluding the proof. For instance, proving that a decider is correct amounts to proving that when the decider says that a Turing machine halts/does not halt, the machine actually halts/does not halt.



% This type of proof, which heavily relies on compute, is known as ``proof by reflexion'' and was originally introduced to prove facts about rings \cite{blanchin}. The \Coq proof of the four color theorem, developed by Gonthier in 2008, also fits in that category where an algorithm is written directly in \Coq, proved correct, and then executed \cite{fourcolours}.



% Although encouraged, the use of ITPs was not imposed by \url{bbchallenge.org} project because a complete formalisation of the result was believed very hard to achieve. \CoqBB came as a suprise in the spring of 2024 when it was released by a new contributor, ``mxdys'', who
