...
This tight link to the halting problem makes $S$ arguably more fundamental\footnote{Also because $\Sigma$ does not generalise straightforwardly to non-binary tape alphabets.} than $\Sigma$, hence we take the liberty (similarly to other authors \cite{BusyBeaverFrontier,otherexamples?}) to focus the busy beaver game on $S$ instead of $\Sigma$ and to call the $n$-state \textit{busy beaver winner} the machine achieving $S(n)$ running steps. Prior to this work, only the four first values of $S$ (and $\Sigma$) had been proved: $S(1)=1$, $S(2)=6$ \cite{Rado_1962}, $S(3) = 21$ \cite{Lin1963}, $S(4) = 107$ \cite{Brady83}.
