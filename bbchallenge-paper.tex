...
\begin{itemize}
    ...
    \item $S(3,3)$: \tm{1RB2RA1LC_2LC1RB2RB_---2LA1LA}; \textbf{Bigfoot}.\footnote{\label{note:bigfoot}\url{https://wiki.bbchallenge.org/wiki/Bigfoot}} Take the following parametrised configuration:
          ...
          The machine enters configuration $B(2,1,2)$ at step 69 and halts if and only if it reaches $B(a,b,c,)$ with $a=0$ and $b \equiv 2 \mod 6$ \cite{ligocki2023bb33}. Bigfoot has also been compiled into a 7-state 2-symbol machine which halts under the same condition: \tm{0RB1RB_1LC0RA_1RE1LF_1LF1RE_0RD1RD_1LG0LG_---1LB}. A probabilistic model\footnotemark[\getrefnumber{note:bigfoot}] suggests that the chance of Bigfoot ever halting are less than $2^{-3999889}$.
    ...
    \item $S(2,5)$: \tm{1RB3RB---3LA1RA_2LA3RA4LB0LB0LA}; \textbf{Hydra}.\footnote{\label{note:hydra}\url{https://wiki.bbchallenge.org/wiki/Hydra}} Take the following parametrised configuration:
          ...
          \[
              \begin{array}{llcll}
                  C(2a,   & 0) & \longrightarrow & \texttt{Halt}                       \\
                  C(2a,   & b) & \longrightarrow & C(3a+3, b-1) & \text{ if } b \geq 1 \\
                  C(2a+1, & b) & \longrightarrow & C(3a+3, b+2)                        \\
              \end{array}
          \]

          The machine reaches $C(3,0)$ after 19 steps and halts if and only if it reaches $C(a,b)$ with $a$ even and $b=0$. This can be reformulated as whether repeatedly applying the \textit{Hydra map} $H(n) = \lfloor \frac{3}{2}n\rfloor$ from $n=3$ will ever reach a point where the number of reached even values is greater than twice the number of reached odd values \cite{ligocki2024bb25}. This is the same Hydra map used for Antihydra above, but with the "opposite" condition (switching the role of even an odd values). A probabilistic model\footnotemark[\getrefnumber{note:hydra}] suggests that the chance of Hydra ever halting are less than $\left( \frac{\sqrt{5} - 1}{2} \right)^{2005374}$.
