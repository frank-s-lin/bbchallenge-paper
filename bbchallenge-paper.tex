...
\newcommand{\noncomput}{non-computable\xspace}
...
\paragraph{Challenges.} Proving $S(5) = \BBtheFifth$ required to analyse the behavior of $\BBtheFifthTNF$ Turing machines\footnote{This exact number can only be known after the proof is done, see Section~\ref{sec:TNF}.}, which in itself is a challenge -- evidently requiring computer assistance. The challenge for a halting machine is the case where it halts after a number of steps that is too enormous to be simulated step-by-step (e.g. the current 6-state champion halts after $10 \uparrow \uparrow 15$ steps, see Appendix~\ref{app:lowerbounds}), this challenge was not met for 5-state machines since they halt in at most $\BBtheFifth$ steps -- this could not have been known in advance but was believed -- which is easy to simulate on modern computers. The challenge for a non-halting machine is that proving it does halt can be hard. How hard?
...
Hence, while 5-state halting machines were not feared, it remained completely unknown how hard could non-halting 5-state machines could get.
