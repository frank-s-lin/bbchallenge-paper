...
\begin{table}[h]
    ...
    \begin{tabular}{c|ccccc}
        ...
        3                & \cellcolor{green!20}$S(2,3) = 38$ \cite{LafittePapazian2007}
                         ...
                         & \cellcolor{orange!20}$S(4,3) > 10^{14{,}072} $
                         & --                                                           & --                                                                        \\
        \hline
    ...
    \caption{Landscape of small busy beaver values.
        \cellcolor{green!20}Cells highlighted in green corresponds to values for which we provide \Coq proofs. Bright green indicates the new results, $S(5)$ and $S(2,4)$, original to this work.
        Cells highlighted in orange indicate the existence of a Cryptid (\ie machines whose halting problem is currently open and believed mathematically hard), see Appendix~\ref{app:cryptids}. Lighter orange means that the existence of a Cryptid comes trivially from reusing a known 3-state 3-symbol Cryptid and ignoring the available additional state or symbol. }
...
\paragraph{Formal Verification (\Coq and \CoqBB).} To the best of our knowledge, our work is the first to formally verify a \BBfull value.
