...
Progress is ongoing in all next Turing machine classes (orange highlight in Table~\ref{table:landscape}). Many new deciders are developed to tackle $S(3,3)$, $S(2,5)$ and $S(6)$, including a generalisation of all the regular CTL deciders presented in this paper (see Section~\ref{sec:deciders}). Most of these new deciders have been  \href{https://github.com/ccz181078/busycoq/tree/BB6/verify}{formalised} using \Coq (Rocq). There currently remains only $6$ holdouts for $S(3)$, including the probviously halting suspected new champion (see Section~\ref{sec:intro:discuss}), and only $83$ holdouts for $S(2,5)$ see our \href{https://wiki.bbchallenge.org/wiki/Holdouts_lists}{lists of holdouts}.

Concerning $S(6)$, the TNF enumeration of 6-state machines, which contains about 33 billion machines, has also been implemented in \Coq (Rocq), and, together with the deciders and about 2,000 individual proofs, ``only'' about 4,000 holdouts remain. Importantly:
\begin{itemize}[label=--]
...
Hence, we can say with absolute certainty that the value of $S(6)$ will never be proved. Nonetheless, \texttt{bbchallenge.org} welcomes all new contributors interested in the Busy Beaver game, independently of background, favorite programming language or proof assistant!
