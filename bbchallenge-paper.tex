...
Progress is ongoing in all unsolved Turing machine classes (orange highlight in Table~\ref{table:landscape}): new deciders are being developed to tackle $S(3,3)$, $S(2,5)$ and $S(6)$, including a generalisation of all the regular CTL deciders presented in this paper (see Section~\ref{sec:deciders}). Most of these new deciders have been  \href{https://github.com/ccz181078/busycoq/tree/BB6/verify}{formalised} using \Coq\,/\,Rocq. There currently remain only $6$ holdouts for $S(3,3)$, including the probviously halting suspected new champion (see Section~\ref{sec:intro:discuss}), and only $83$ holdouts for $S(2,5)$; see our \href{https://wiki.bbchallenge.org/wiki/Holdouts_lists}{lists of holdouts}.
...
Hence, in perpetuating a longstanding tradition of hope (see \rado's and Brady's quotes in the incipit of this paper), we predict that $S(6)$ will never be proved.\footnote{Nonetheless, \texttt{bbchallenge.org} welcomes all new contributors interested in the Busy Beaver game!}
