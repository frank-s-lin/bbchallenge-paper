...
Each enumerated machine is fed through a sequence (or \textit{pipeline}) of \textit{deciders}, where a decider is an algorithm deciding, if it can, whether the machine halts or not. Because of the noncomputability of the halting problem, there is no \textit{universal} decider able to decide \textit{any} Turing machine. Hence, all the craft resides in creating deciders able to decide large families of machines, in reasonable time. The $S(5)$ pipeline of deciders is given in Figure~\ref{fig:pipelineS5} (see Figure~\ref{fig:pipelineS2x4} for $S(2,4)$). Deciders are presented in Section~\ref{sec:deciders}. Apart from \textit{loops} (Section~\ref{sec:loops}) \cite{Lin1963}, all our deciders are original.

In the case of 5-state machines, \numSporadic machines are not solved by our deciders and required individual proofs of nonhalting. We call them \textit{sporadic} machines and they are presented in Section~\ref{sec:sporadic}.
