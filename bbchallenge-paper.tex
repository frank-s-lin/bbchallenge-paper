...
\begin{restatable}[\CoqBB\footnote{\url{https://github.com/ccz181078/Coq-BB5}}: \texttt{Lemma BB5\_value}]{theorem}{thBBTheFifth}\label{th:BB5}
    $S(5) = \BBtheFifth$.
\end{restatable}
...
\paragraph{So what?} The Busy Beaver game is a relatively polarizing scientific topic: some love it, some hate it. Common arguments against studying this problem are of a utilitarian nature: what is the \textit{point}, what is the \textit{use}? We leave the floor to Scott Aaronson \cite{SAblogquote}:
...
\begin{quote}
    ...
    \hfill --- Scott Aaronson \cite{SAblogquote}
\end{quote}

The Busy Beaver game is a concrete attempt at clearly identifying the frontier between the knowable and the unknowable. In all the next Turing machines classes (orange highlight in Table~\ref{table:landscape}), we have found what we call ``Cryptids'' which are loosely defined as Turing machines whose halting problem from the all-zero tape is believed to be mathematically hard (Appendix~\ref{app:cryptids}); for instance, with 6 states, \href{https://bbchallenge/1RB1RA_0LC1LE_1LD1LC_1LA0LB_1LF1RE_---0RA}{Antihydra} is a machine that does not halt from the all-zero tape if and only if the following conjecture holds\footnote{The conjecture is believed to be true because a probabilistic \href{https://wiki.bbchallenge.org/w/index.php?title=Antihydra}{analysis} of the Turing machine suggests that its probability of ever halting is minuscule, namely, ${\left(\frac{\sqrt{5}-1}{2}\right)}^{1073720885}\approx 4.841\times 10^{-224394395}$. The map $H$ is also known to not generate Sturmian words \cite{DUBICKAS_2009}.}:


\begin{conjecture}[Antihydra]
...
Antihydra is the \textit{smallest}\footnote{Or, to be exact, one of the smallest since there are other 6-state Cryptids such as this non-Collatz \href{https://wiki.bbchallenge.org/wiki/1RB1RE_1LC0RA_0RD1LB_---1RC_1LF1RE_0LB0LE}{example}. We also refrained of saying the \textit{simplest} since there are certainly machines with open halting problems which are simpler to solve.} open problem in mathematics, on the Busy Beaver scale. Antihydra is marked as a Cryptid, \ie is believed mathematically hard, because of its resemblance with the notoriously hard Collatz conjecture \cite{LagariasCollatz}.

Beyond Cryptids, the Busy Beaver game, by means of exhaustive enumeration (see Section~\ref{sec:enum}), enables the discovery of \textit{wild} algorithms -- \eg Antihydra, or the Sporadic Machines, see Section~\ref{sec:sporadic}. As computer scientists, this generates the same childlike wonder as biologists may feel discovering new life forms or astronomers new planets, which to us, defies all utilitarian concerns.
...
In any case, we can say with absolute certainty that the value of $S(6)$ will never be proved.
