...
\paragraph{Related Work.} The proof of $S(4) = \BBtheFourth$ was published in 1983 \cite{Brady83}.  A caveat of the proof resides in the following quote: ``All of the remaining holdouts were examined by means of voluminous printouts of their histories along with some program extracted features. It was determined to the author's satisfaction that none of these machines will ever stop.'' -- where \textit{holdout} means a machine still needing a proof of halting/nonhalting. Using the Coq theorem prover, we confirm that the result is correct, see Theorem~\ref{th:BB4}. One of the main innovations in the original proof of $S(4) = \BBtheFourth$ was the introduction of a method to solve the halting problem of machines that the author calls \textit{XMas Trees} \cite{Brady83}. While we developed a similar, slighlty generalised concept (see Appendix~\ref{app:bouncers}), this technique ended up not being used in our computer-verified proofs of $S(5)$ and $S(4)$.
