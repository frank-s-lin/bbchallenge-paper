...
A \textit{decider} is a program that takes as input a Turing machine $\mathcal{M}$ and that returns either \HALT, \NONHALT, \UNKNOWN whether it was able to detect that the machine's halting status (halt or nonhalt) or not. A \textit{pipeline} of deciders is several deciders applied in sequence: a machine is tested by each decider successively until one of them outputs \HALT or \NONHALT.
...
\begin{table}[h!]
    ...
    \begin{tabular}{llll}
        Decider                                                                              & Nonhalt                         & Halt                           & Total decided \\
        \hline
        ...
        7. Sporadic machines, see \textbf{Section~\ref{sec:sporadic}}                        & 14                              & 0                              & 14            \\ \hline
