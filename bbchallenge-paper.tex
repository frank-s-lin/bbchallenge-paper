...
\begin{itemize}
    ...
    \item $S(3) = 21$ is achieved by 1 winner: \tm{1RB1RZ_1LB0RC_1LC1LA}; $\Sigma(3) = 6$ is achieved by 5 winners: \tm{1RB1RZ_0RC1RB_1LC1LA}, \tm{1RB1RC_1LC1RZ_1RA0LB}, \tm{1RB1LC_1LA1RB_1LB1RZ}, \tm{1RB1RA_1LC1RZ_1RA1LB}, and \tm{1RB1RA_1LC1RZ_1RA1LB}. Note that this is the only known case where no $S$ winner is a $\Sigma$ winner. Originally proved in \cite{Lin1963}.
    ...
    \item $S(2,3) = 38$ is achieved by 1 winner, \tm{1RB2LB1RZ_2LA2RB1LB}, which is also the only machine for $\Sigma(2,3) = 9$. Originally proved in \cite{LafittePapazian2007}.
...
\paragraph{Champions.} Below are the current Busy Beaver champions for some of the next Busy Beaver values that are still unknown, \ie machines achieving higher $S$ scores could be found in the future. Note that for most of these champions computing the exact $S$ is generally harder than proving that the machine halts which and most of these bounds do not correspond to an \textit{exact} number of steps.\footnote{Also, at these scales, $S$ and $\Sigma$ are believed to be roughly the same number since, experimentally, $\Sigma$ is always ``only'' quadratically smaller than $S$.}
\begin{itemize}
    ...
    \item $S(3,3) \geq 119{,}112{,}334{,}170{,}342{,}541$; this bound is the exact number of stepped performed by \tm{0RB2LA1RA_1LA2RB1RC_1RZ1LB1LC}, yielding the simplified bound $S(3,3) > 10^{17}$. Only 6 unsolved 3-state 3-symbol Turing machines currently remain including \tm{1RB2LC1RC_2LC---2RB_2LA0LB0RA} which is believed to halt and which number of steps would largely surpass the current bound, see Section~\ref{sec:fw}.\footnote{https://wiki.bbchallenge.org/wiki/BB(3,3)}
    ...
    \item $S(3,4) > 2 \uparrow^{15} 5$; this bound comes from \tm{1RB3LB1RZ2RA_2LC3RB1LC2RA_3RB1LB3LC2RC}. For this machine it is possible to prove that the number of \sone symbols on final tape is \textit{exactly} $(2 \uparrow^{15} 5) + 14$, see \cite{ligocki2024bb34ack14}.
