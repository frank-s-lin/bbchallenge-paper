...
The function $S$ can naturally be extended to Turing machines using more than two alphabet symbols \cite{BradyMeaningOfLife}, i.e. $S(5) = S(5,2)$. It was previously known that $S(2,3) = 38$ \cite{BradyMeaningOfLife, MICHEL200445, LafittePapazian2007}. We prove, using the Coq theorem prover, that $S(2,4) = \BBTxF$, see Theorem~\ref{th:BB24}.

\begin{theorem}\label{th:BB24}
    $S(2,4) = \BBTxF$.
\end{theorem}

That way, we have now a clearer view of the small Busy Beaver values landscape, see Table~\ref{table:landscape}.
