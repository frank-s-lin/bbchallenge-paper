...
The winning machine for $S$ is also the winning machine for $\Sigma$ and we prove $\Sigma(5) = \SigmaTheFifth$, see Theorem~\ref{th:BB5Sigma}. Also, the function $S$ can naturally be extended to Turing machines using more than two alphabet symbols \cite{BradyMeaningOfLife}, where, for instance $S(5,2)$ is the value of $S$ for 5-state, 2-symbols machines, \ie $S(5)$. It was previously known that $S(2,3) = 38$ \cite{BradyMeaningOfLife, MICHEL200445, LafittePapazian2007}. We also prove, using the Coq theorem prover \cite{the_coq_development_team_2024_14542673}, that $S(2,4) = \BBTxF$, see Theorem~\ref{th:BB24}.
